...
namespace P4EXP
{
	public class P4ObjectTreeListViewItem : TreeListViewItem
	{
		...
		public static string PrettyPrintFileSize(long fileSize)
		{
			...
			if (fileSize < 1024)
			{
				return string.Format(Properties.Resources.SccHistoryDetailsControl_FileSizeBytes, fileSize);
			}
			else if (fileSize < mBytes)
			{
				fSize = fSize / kBytes;
				return string.Format(Properties.Resources.SccHistoryDetailsControl_FileSizeKiloBytes, fSize);
			}
			else if (fileSize < gBytes)
			{
				fSize = fSize / mBytes;
				return string.Format(Properties.Resources.SccHistoryDetailsControl_FileSizeMegaBytes, fSize);
			}
			else if (fileSize < tBytes)
			{
				fSize = fSize / gBytes;
				return string.Format(Properties.Resources.SccHistoryDetailsControl_FileSizeGigaBytes, fSize);
			}
			fSize = fSize / tBytes;
			return string.Format(Properties.Resources.SccHistoryDetailsControl_FileSizeTeraBytes, fSize);
		}
