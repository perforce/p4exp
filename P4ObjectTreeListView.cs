...
namespace P4EXP
{
	public partial class P4ObjectTreeListView : TreeListView
	{
		...
		public void InitView(IList<FileMetaData> files, IList<object> fields)
		{
			Files = files;
			Fields = fields;
		...
		IList<FileMetaData> Files { get; set; }
		...
		public IList<FileMetaData> SelectedFileList
		{
			get
			{
			...
			set
			{
				if (CheckBoxes)
				{
					...
					foreach (FileMetaData file in value)
					{
		...
		[DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
		public IList<FileMetaData> UnselectedFileList
		{
			get
			{
				List<FileMetaData> value = new List<FileMetaData>();
